...
INTEGER(KIND=JPIM) :: JG,JLEV,JLON
...
IF ( ANY(FGTS_OA /= 0._JPRB) ) THEN
  ...
  DO JG=1,3
        DO JLON=KIDIA,KFDIA
            !IF ( LDMASKS(JLON) ) THEN
        ZTAU(JLON,JG)=1._JPRB-ZAR(JLON,JG)-ZCOEF(JLON,JG)*FGTS_OA(JG)*&
         & (1._JPRB-ZAR(JLON,JG))**FGTS_OB(JG)*ZAR(JLON,JG)**FGTS_OC(JG)*&
         & (1._JPRB-FGTS_OD(JG)*ZAR(JLON,JG))
      !ENDIF
    ENDDO
  ENDDO
