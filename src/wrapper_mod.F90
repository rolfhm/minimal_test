...
SUBROUTINE WRAPPER(KLON,KLEV,KGPBLK,LCHECK)
...
! & PGDEOSI, &
! & PGUEOSI, &
! & PGMU0,PGMU0_MIN,PGMU0_MAX, &
! & PGDEOTI,PGDEOTI2, &
! & PGUEOTI,PGUEOTI2, &
! & PGEOLT,PGEOXT, &
! & PGRPROX, &
! & PGMIXP,PGFLUXC,PGRSURF, &
 & PSDUR, &
 & PFRSO,PFRTH, &
 & PFRSODS,PFRSOPS,PFRSOLU,PFRTHDS, &
 & PDAER)

call system_clock(count=scount, count_rate=c_rate)
    ...
    DO JBLK=1,KGPBLK
      ...
      !DO JLON=1,KLON
        ...
        !   KFDIA=JLON
            ...
            ! - INPUT 2D
             & KSTEP,PAPRS(:,:,JBLK),PAPRSF(:,:,JBLK),PCP(:,:,JBLK),PR(:,:,JBLK),PDELP(:,:,JBLK),PNEB(:,:,JBLK), &
...
call system_clock(count=ecount, count_rate=c_rate)
t = real(ecount - scount,8)/c_rate
write (*,'(A,F8.2,A)') 'elapsed time : ', t, ' s'
write (*,'(A,F8.4,A)') '          i.e. ', 1000.*t/real(KLON*KGPBLK,8), ' ms/gp'
write (*,'(A,F8.4,A)') '          i.e. ', real(KLON*KGPBLK,8)/(1000.*t), ' gp/ms'
